...
def load_train_objs(optimizer_choice):
...
def main(save_every: int, total_epochs: int, opt: str, snapshot_path: str = "snapshot.pt"):
    dist.init_process_group(backend='nccl')
    model, optimizer = load_train_objs(opt)
...
if __name__ == "__main__":
    ...
    if not rank:
        print("--------------------------------")
        print(f"optimazer is {opt}")
    start_time = time.time()
    main(SAVE_EVERY, NUM_EPOCHS, opt)
    end_time = time.time()
